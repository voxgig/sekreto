/* What the library's own files share and a consumer never sees: the
 * string helpers, the hand-rolled crypto, the HTTP transport and the TLS
 * seam.
 *
 * The TLS seam is one function wide on purpose. `sek_tls_*` is declared
 * here and implemented in tls.c, and tls.c is the only file in this port
 * that names OpenSSL: `nm` proves it, and `make check-tls` in the
 * Makefile is that proof run.
 */
