...
sekreto is one library written twenty times. The rules below exist so
those twenty stay one.
...
2. **`spec/sekreto.aon` is the contract.** It runs against every port. A
   ...
   spec. Changing the spec means changing twenty ports, so change it
   deliberately.
...
Twenty ports, **two of which publish a package**:
...
The other eighteen (python, ruby, php, perl, go, rust, java, csharp, zig,
kotlin, scala, clojure, swift, dart, elixir, cpp, c, lua) ship no package
and have no publish flow yet — they are consumed from this repository.
