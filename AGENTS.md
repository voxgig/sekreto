...
sekreto is one library written twenty-three times. The rules below exist
so those twenty-three stay one.
...
2. **`spec/sekreto.aon` is the contract.** It runs against every port. A
   ...
   spec. Changing the spec means changing twenty-three ports, so change it
   deliberately.
...
Twenty-three ports, **two of which publish a package**:
...
The other twenty-one (python, ruby, php, perl, go, rust, java, csharp,
zig, kotlin, scala, clojure, swift, dart, elixir, cpp, c, lua, ocaml,
haskell, lean) ship no package and have no publish flow yet — they are
