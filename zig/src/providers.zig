...
pub fn checkaddr(alloc: Allocator, addr: []const u8) Allocator.Error!Answer(void) {
    if (std.mem.startsWith(u8, addr, "https://")) {
        return .{ .ok = {} };
    }
    ...
    const rest = addr["http://".len..];
    const authority_end = std.mem.indexOfAny(u8, rest, "/?#") orelse rest.len;
    var authority = rest[0..authority_end];

    // Userinfo is not the host: `http://127.0.0.1@evil.com/` reaches
    // evil.com, and must be refused as such.
    if (std.mem.lastIndexOfScalar(u8, authority, '@')) |at| {
        authority = authority[at + 1 ..];
    }

    // A bracketed IPv6 literal keeps its brackets; splitting the authority
    // on the first colon would yield `[` and refuse a legitimate local vault
    // at `http://[::1]:8200`.
    const host = if (0 != authority.len and '[' == authority[0]) blk: {
